...
def get_feature_column(data: pd.DataFrame) -> list[str]:
    excluded_columns = {
        "date",
        TARGET_COLUMN,
...
def plot_training_history(history: dict[str, list[float]]) -> None:
    ...
    plt.plot(epochs, history["val_loss"], label="validatin loss")
    plt.title("EfficientKAN Training History")
    plt.xlabel("Epoch")
    plt.ylabel("MSE loss")
    plt.legend()
    plt.grid(True)
    ...
    output_path = FIGURES_DIR / "kan_training_history.png"
...
def main() -> None:
    print("Training EfficientKAN volatility model")
    print("======================================")

    data = load_dataset()
    train, val, test = chronological_split(data)
    feature_columns = get_feature_column(data)

    print(f"Dataset rows: {len(data)}")
    print(f"Feature columns: {len(feature_columns)}")
    print()
    print("Split data ranges:")
    ...
    model_name = "efficient_kan_16_8_grid5"
    ...
    summary_path = TABLES_DIR / "kan_model_summary.csv"
    predictions_path = TABLES_DIR / "kan_test_predictions.csv"
    ...
    print()
    print("EfficientKAN training complete.")
    print()
    print("EfficientKAN summary:")
